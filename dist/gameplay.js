...
class ChessGame {
    constructor(players) {
        this.board = new chessboard_1.ChessBoard();
        this.board.initialize();
        this.currentTurn = 'white';
        this.chessPlayers = this.playerCoinToss(players);
        this.whiteKing = this.board.getPiece([7, 4]);
        this.blackKing = this.board.getPiece([0, 4]);
        // Initially disable gameplay
        this.gameOver = true;
    }
    playerCoinToss(players) {
    ...
    displayBoard(playerColor = 'white') {
    ...
    movePiece(player, start, end) {
        if (this.gameOver)
            return [false, ""];
        if (player !== this.getCurrentTurnPlayer())
            return [false, "It's not currently your turn."];
